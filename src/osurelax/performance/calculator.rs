...
use crate::{
    osurelax::{
        ...
        OsuRelaxDifficultyAttributes, OsuRelaxPerformanceAttributes, OsuRelaxScoreState,
    },
    util::{
        difficulty::{erf, erf_inv, reverse_lerp},
        float_ext::FloatExt,
    },
    GameMods,
};
...
pub(super) struct OsuRelaxPerformanceCalculator<'mods> {
...
impl<'a> OsuRelaxPerformanceCalculator<'a> {
    pub const fn new(
        attrs: OsuRelaxDifficultyAttributes,
        mods: &'a GameMods,
        acc: f64,
...
impl OsuRelaxPerformanceCalculator<'_> {
    pub fn calculate(self) -> OsuRelaxPerformanceAttributes {
        let total_hits = self.state.total_hits();

        if total_hits == 0 {
            return OsuRelaxPerformanceAttributes {
...
const fn total_successful_hits(state: &OsuRelaxScoreState) -> u32 {
