...
/// and it will return the resulting current [`OsuRelaxPerformanceAttributes`].
/// To process multiple objects at once, use [`nth`] instead.
///
/// Both methods require an [`OsuRelaxScoreState`] that contains the current
/// hitresults as well as the maximum combo so far.
///
/// If you only want to calculate difficulty attributes use
/// [`OsuRelaxGradualDifficulty`] instead.
///
/// # Example
///
/// ```
...
/// let mut gradual = OsuRelaxGradualPerformance::new(difficulty, &map).unwrap();
/// let mut state = OsuRelaxScoreState::new(); // empty state, everything is on 0.
///
...
/// ```
///
/// [`next`]: OsuRelaxGradualPerformance::next
/// [`nth`]: OsuRelaxGradualPerformance::nth
pub struct OsuRelaxGradualPerformance {
    lazer: bool,
    difficulty: OsuRelaxGradualDifficulty,
}

impl OsuRelaxGradualPerformance {
    ...
    pub fn new(difficulty: Difficulty, map: &Beatmap) -> Result<Self, ConvertError> {
        ...
        let difficulty = OsuRelaxGradualDifficulty::new(difficulty, map)?;
    ...
    pub fn next(&mut self, state: OsuRelaxScoreState) -> Option<OsuRelaxPerformanceAttributes> {
        self.nth(state, 0)
    }

    /// Process all remaining hit objects and calculate the final performance
    /// attributes.
    pub fn last(&mut self, state: OsuRelaxScoreState) -> Option<OsuRelaxPerformanceAttributes> {
...
mod tests {
    ...
    fn next_and_nth() {
        ...
        let mut gradual = OsuRelaxGradualPerformance::new(difficulty.clone(), &map).unwrap();
        let mut gradual_2nd = OsuRelaxGradualPerformance::new(difficulty.clone(), &map).unwrap();
        let mut gradual_3rd = OsuRelaxGradualPerformance::new(difficulty.clone(), &map).unwrap();

        let mut state = OsuRelaxScoreState::default();
        ...
        for i in 1.. {
            ...
            let mut regular_calc = OsuRelaxPerformance::new(&map)
