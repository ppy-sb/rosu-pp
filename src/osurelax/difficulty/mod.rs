...
use crate::{
    ...
    osurelax::{
        ...
        difficulty::{object::OsuRelaxDifficultyObject, scaling_factor::ScalingFactor},
...
use super::attributes::OsuRelaxDifficultyAttributes;
...
pub fn difficulty(
    ...
    map: &Beatmap,
) -> Result<OsuRelaxDifficultyAttributes, ConvertError> {
...
pub fn checked_difficulty(
    difficulty: &Difficulty,
    map: &Beatmap,
) -> Result<OsuRelaxDifficultyAttributes, CalculateError> {
    let map = map.convert_ref(GameMode::Osu, difficulty.get_mods())?;
    map.check_suspicion()?;
...
pub struct OsuRelaxDifficultySetup {
...
pub struct DifficultyValues {
    pub skills: OsuRelaxSkills,
    pub attrs: OsuRelaxDifficultyAttributes,
}
...
impl DifficultyValues {
    pub fn calculate(difficulty: &Difficulty, map: &Beatmap) -> Self {
        ...
        let mut skills = OsuRelaxSkills::new(mods, &scaling_factor, &map_attrs, time_preempt);
    ...
    pub fn create_difficulty_objects<'a>(
        difficulty: &Difficulty,
        scaling_factor: &ScalingFactor,
        osu_objects: impl ExactSizeIterator<Item = Pin<&'a mut OsuRelaxObject>>,
    ) -> Vec<OsuRelaxDifficultyObject<'a>> {
        let take = difficulty.get_passed_objects();
        let clock_rate = difficulty.get_clock_rate();

        let mut osu_objects_iter = osu_objects
            .map(|h| OsuRelaxDifficultyObject::compute_slider_cursor_pos(h, scaling_factor.radius))
            .map(Pin::into_ref);
        ...
        osu_objects_iter
            .enumerate()
            .map(|(idx, h)| {
                let diff_object = OsuRelaxDifficultyObject::new(
                    h.get_ref(),
                    last.get_ref(),
                    last_last.as_deref(),
                    clock_rate,
