...
impl<'a> OsuRelaxDifficultyObject<'a> {
    ...
    pub fn new(
        hit_object: &'a OsuRelaxObject,
        last_object: &'a OsuRelaxObject,
        last_last_object: Option<&OsuRelaxObject>,
        clock_rate: f64,
        idx: usize,
    ...
    fn set_distances(
        &mut self,
        last_object: &OsuRelaxObject,
        last_last_object: Option<&OsuRelaxObject>,
        clock_rate: f64,
        scaling_factor: &ScalingFactor,
    ) {
        if let OsuRelaxObjectKind::Slider(ref slider) = self.base.kind {
            ...
            self.travel_time = (self.base.lazy_travel_time() / clock_rate)
                .max(OsuRelaxDifficultyObject::MIN_DELTA_TIME);
        }
        ...
        if let OsuRelaxObjectKind::Slider(ref last_slider) = last_object.kind {
            let last_travel_time = (last_object.lazy_travel_time() / clock_rate)
                .max(OsuRelaxDifficultyObject::MIN_DELTA_TIME);
            self.min_jump_time =
                (self.strain_time - last_travel_time).max(OsuRelaxDifficultyObject::MIN_DELTA_TIME);
    ...
    /// The [`Pin<&mut OsuRelaxObject>`](std::pin::Pin) denotes that the object will
    /// be mutated but not moved.
    pub fn compute_slider_cursor_pos(
        mut h: Pin<&mut OsuRelaxObject>,
        radius: f64,
    ) -> Pin<&mut OsuRelaxObject> {
        let pos = h.pos;
        let stack_offset = h.stack_offset;
        let start_time = h.start_time;

        let OsuRelaxObjectKind::Slider(ref mut slider) = h.kind else {
            return h;
        };
        ...
        let scaling_factor = f64::from(OsuRelaxDifficultyObject::NORMALIZED_RADIUS) / radius;
        ...
        for (curr_movement_obj, i) in nested.iter().zip(1..) {
            ...
            let mut required_movement = f64::from(OsuRelaxDifficultyObject::ASSUMED_SLIDER_RADIUS);
            ...
            if i == nested.len() {
                ...
                required_movement = f64::from(OsuRelaxDifficultyObject::NORMALIZED_RADIUS);
            }
    ...
    const fn get_end_cursor_pos(hit_object: &OsuRelaxObject) -> Pos {
        if let OsuRelaxObjectKind::Slider(ref slider) = hit_object.kind {
...
impl IDifficultyObject for OsuRelaxDifficultyObject<'_> {
...
impl HasStartTime for OsuRelaxDifficultyObject<'_> {
