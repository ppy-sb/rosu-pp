...
pub struct OsuRelaxObject {
    pub pos: Pos,
    pub start_time: f64,
    pub stack_height: i32,
    pub stack_offset: Pos,
    pub kind: OsuRelaxObjectKind,
}
...
impl OsuRelaxObject {
    ...
    pub fn reflect_vertically(&mut self) {
        ...
        if let OsuRelaxObjectKind::Slider(ref mut slider) = self.kind {
    ...
    pub fn end_time(&self) -> f64 {
        match self.kind {
            OsuRelaxObjectKind::Circle => self.start_time,
            OsuRelaxObjectKind::Slider(ref slider) => slider.end_time,
            OsuRelaxObjectKind::Spinner(ref spinner) => self.start_time + spinner.duration,
        }
    }
    ...
    pub fn end_pos(&self) -> Pos {
        match self.kind {
            OsuRelaxObjectKind::Circle | OsuRelaxObjectKind::Spinner(_) => self.pos,
            OsuRelaxObjectKind::Slider(ref slider) => {
    ...
    pub const fn lazy_travel_time(&self) -> f64 {
        match self.kind {
            OsuRelaxObjectKind::Circle | OsuRelaxObjectKind::Spinner(_) => 0.0,
            OsuRelaxObjectKind::Slider(ref slider) => slider.lazy_travel_time,
        }
    }

    pub const fn is_circle(&self) -> bool {
        matches!(self.kind, OsuRelaxObjectKind::Circle)
    }

    pub const fn is_slider(&self) -> bool {
        matches!(self.kind, OsuRelaxObjectKind::Slider { .. })
    }

    pub const fn is_spinner(&self) -> bool {
        matches!(self.kind, OsuRelaxObjectKind::Spinner(_))
    }
}

pub enum OsuRelaxObjectKind {
    Circle,
    Slider(OsuSlider),
    Spinner(Spinner),
}
...
impl OsuSlider {
    fn new(
        ...
        let velocity = f64::from(OsuRelaxObject::BASE_SCORING_DIST) * slider_multiplier
