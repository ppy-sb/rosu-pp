...
use super::{
    attributes::OsuRelaxDifficultyAttributes,
    difficulty::scaling_factor::ScalingFactor,
    object::{NestedSliderObjectKind, OsuRelaxObject, OsuRelaxObjectKind},
};
...
pub fn convert_objects(
    ...
    let mut osu_objects: Box<[_]> = map
        .hit_objects
        .iter()
        .map(|h| OsuRelaxObject::new(h, map, reflection, &mut curve_bufs, &mut ticks_buf))
        .inspect(|h| {
            if take == 0 {
                return;
            }
            ...
            match h.kind {
                ...
                OsuRelaxObjectKind::Spinner(_) => attrs.n_spinners += 1,
            }
        })
        .collect();
    ...
    match reflection {
        ...
        Reflection::Horizontal => osu_objects
            .iter_mut()
            .for_each(OsuRelaxObject::reflect_horizontally),
        Reflection::Both => osu_objects
            .iter_mut()
            .for_each(OsuRelaxObject::reflect_both_axes),
    }
    ...
    for h in osu_objects.iter_mut() {
        ...
        if let OsuRelaxObjectKind::Slider(ref mut slider) = h.kind {
...
fn stacking(hit_objects: &mut [OsuRelaxObject], stack_threshold: f64) {
...
fn old_stacking(hit_objects: &mut [OsuRelaxObject], stack_threshold: f64) {
    for i in 0..hit_objects.len() {
        ...
        let pos2 = {
            ...
            match h.kind {
                OsuRelaxObjectKind::Circle | OsuRelaxObjectKind::Spinner(_) => h.pos,
                OsuRelaxObjectKind::Slider(ref slider) => {
