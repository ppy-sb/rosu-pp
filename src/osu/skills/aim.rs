...
impl StrainSkill for Aim {
    ...
    fn strain_value_at(
        ...
        let total_strain = if self.with_rx { self.curr_strain * self.curr_rhythm.powf(2.5).max(2.0).min(1.0) } else { self.curr_strain };
...
impl AimEvaluator {
    ...
    fn evaluate_diff_of(
        ...
        let acute_angle_multiplier = if with_rx { Self::ACUTE_ANGLE_MULTIPLIER * 1.35 } else { Self::ACUTE_ANGLE_MULTIPLIER };
        let wide_angle_multiplier = if with_rx { Self::WIDE_ANGLE_MULTIPLIER * 1.05 } else { Self::WIDE_ANGLE_MULTIPLIER };
        let slider_multiplier = if with_rx { Self::SLIDER_MULTIPLIER * 4.0 } else { Self::SLIDER_MULTIPLIER };
        let velocity_multiplier = if with_rx{ Self::VELOCITY_CHANGE_MULTIPLIER * 1.25 } else { Self::VELOCITY_CHANGE_MULTIPLIER }; 
        ...
        if with_rx {
            ...
            // Starins Time reflects how fast the object is
            let mut strain_time = curr.strain_time;
            strain_time /= ((strain_time / hit_window) / 0.94).clamp(0.93, 1.0);

            // Single Spacing Threshold reflects the minimum distance of that "spacing" stream
            // It can also be considered as how hard to aim the stream is. 
