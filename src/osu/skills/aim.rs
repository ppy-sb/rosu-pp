...
impl StrainSkill for Aim {
    ...
    fn strain_value_at(
        ...
        let total_strain = self.curr_strain * self.curr_rhythm.powf(1.8).max(2.0).min(1.0);
...
impl AimEvaluator {
    ...
    fn evaluate_diff_of(
        ...
        let slider_multiplier = if with_rx { Self::SLIDER_MULTIPLIER * 1.4 } else { Self::SLIDER_MULTIPLIER };
        let velocity_multiplier = if with_rx{ Self::VELOCITY_CHANGE_MULTIPLIER * 1.2 } else { Self::VELOCITY_CHANGE_MULTIPLIER } ; 
