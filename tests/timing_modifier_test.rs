...
fn timing_modifier_rewards_tight_timing() {
    ...
    if std::path::Path::new(map_path).exists() {
        ...
        println!(
            "Perfect score - sigma: {:.2}ms, multiplier: {:.4}, pp: {:.2}",
            perfect_perf.timing_core_sigma, perfect_perf.acc_multiplier, perfect_perf.pp
        );
        println!(
            "Good score    - sigma: {:.2}ms, multiplier: {:.4}, pp: {:.2}",
            good_perf.timing_core_sigma, good_perf.acc_multiplier, good_perf.pp
        );
        println!(
            "Sloppy score  - sigma: {:.2}ms, multiplier: {:.4}, pp: {:.2}",
            sloppy_perf.timing_core_sigma, sloppy_perf.acc_multiplier, sloppy_perf.pp
        );

        // Assertions:
        // 1. Tighter timing should have higher multiplier
        assert!(
            perfect_perf.acc_multiplier >= good_perf.acc_multiplier,
            "Perfect score should have higher timing multiplier than good score"
        );
        assert!(
            good_perf.acc_multiplier >= sloppy_perf.acc_multiplier,
            "Good score should have higher timing multiplier than sloppy score"
        );

        // 2. Fitted sigma should reflect score quality
        assert!(
            perfect_perf.timing_core_sigma < good_perf.timing_core_sigma,
            "Perfect score should have tighter sigma than good score"
        );
        assert!(
            good_perf.timing_core_sigma < sloppy_perf.timing_core_sigma,
            "Good score should have tighter sigma than sloppy score"
        );

        // 3. PP should increase with tighter timing
        assert!(
            perfect_perf.pp > sloppy_perf.pp,
...
fn timing_modifier_basic_sanity() {
    ...
    for (fitted_sigma, expected_mult, desc) in test_cases {
        ...
        println!(
            "  sigma={:5.1}ms -> ratio={:.3}, mult={:.3} ({})",
            fitted_sigma, ratio, multiplier, desc
        );

        assert!(
            (multiplier - expected_mult).abs() < 0.001,
